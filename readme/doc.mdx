...
Nous allons générer successivement les $G_n := \{m \in \Sigma^\star \mid S (\bigcup_{0 \leq k \leq n} \->^k) m\}$ à l'aide de $G_{n+1} = \bigcup_{x \in G_n} \mathcal{S}(x)$ où $\mathcal{S}(x) := \{m \in \Sigma^\star \mid x \-> m \} = \bigcup_{d \in D} \{m \in \Sigma^\star \mid x \overset{d}{\->} m \}$
...
!nbcode preprocessgf et succ qui calcule $\mathcal{S}(x)$!
...
Rappelons, le problème : étant donné un mot $m \in \delta(S)$, comment le dériver en un mot $m' \in \delta(m)$ donné ?
Pour tout $l \in \Sigma$, on va chercher un ensemble $q(l)$ qui majore $\norm{\delta(m)}_l$ ($=\{\norm{x}_l \mid x \in \delta(m)\}$), l'ensemble des nombres d'occurence de $l$ dans les successeurs de $m$.
...
\fbox{si pour un mot $x \in \delta(m)$, $\exists l \in \Sigma, \norm{\delta(x)}_l \cap q(l) = \emptyset$,
 alors $m' \notin \delta(x)$}.

Pour calculer $\norm{\delta(x)}_l \cap q(l)$, nous allons utiliser un graphe semblable à un automate que nous noterons $A_0$.
...
#### $\overset{*}{\->} := \bigcup_{n\in \mathbb{Z}}$ est la cloture transitive réflexive et symétrique de $\->$, $x\overset{*}{\->} x'$ si $x'$ dérive depuis $x$. ($x'\in \delta(x)$)
