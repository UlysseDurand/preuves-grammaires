# TIPE Sur la vérification de preuve et la preuve automatique d'appartenance d'un mot à une grammaire.
...
!bExemple :! la grammaire des expressions arithmétiques suffixes.
---ml
...
On part des $\\{ [B(b_1);\dots;B(b_n)] | (a,b)\in D \text{ et } b \in T^\star , n = |b|\\}$ et on construit d'autres arbres de dérivation à partir des règles de dérivation.
...
On génère en effet, avec cette méthode, à la $n$-ième étape, $A_n = \\{m \in (N \cup T)^\star \mid  S \(\bigcup_{k \in [1,n]} \rightarrow^k\) m\\}$.
