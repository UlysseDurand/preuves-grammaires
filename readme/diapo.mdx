...
${m_k}_{\mid [1,{\color{blue} j_k} - 1]} {\color{red} a } {m_k}_{\mid [\norm{a_{{\color{blue} i_k}}} + {\color{blue} j_k},\norm{m_k}]} \overset{(a_{{\color{blue} i_k}},b_{{\color{blue} i_k}})}{\->} {m_k}_{\mid [1,{\color{blue} j_k} - 1]} {\color{red} b } {m_k}_{\mid [\norm{a_{{\color{blue} i_k}}} + {\color{blue} j_k},\norm{m_k}]}$
...
---f-Calcul des arrêtes $A$ du graphe-
Pour un sommet de départ fixé et une dérivation fixée.
$A_{q,d}(\Sigma) := \{(q,b) \in A \mid \exists x,x' \in \Sigma^\star / x \overset{d}{\->} x' \text{ et } cat(x) = q \text{ et } cat(x') = q'\}$

Soit $l \in \Sigma$, $\Sigma ' := \Sigma \backslash \{l\}$, $A = \bigcup_{q \in Q}  \bigcup_{d \in D} A_{q,d}(\Sigma)$.

---i
#### $cat(b)(l) = \mathbb{N}^\star$
